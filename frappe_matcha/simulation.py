...
class Simulation:
	def __init__(self,
				...
				interval=7, max_donation_qty=1000, writeFiles=False):
		# Data
		...
		try:
			...
			try:
				self.distance_mat = pickle.load(open(os.path.join(dirname, recipient_path), "rb"))
			except Exception as e:
				logger.error(e)
				logger.error(f'Cannot load distance matrix {os.path.join(dirname, recipient_path)}')
