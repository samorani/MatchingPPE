...
# # date is the current date
# # curdon is the dataframe of current donors with the given ppe (don_id,date,ppe,qty). The same donor could appear multiple times
# # currec is the dataframe of current recipients with the given ppe (rec_id,date,ppe,qty). The same recipient could appear multiple times
# # D: distance matrix (don,rec,distance)
# # returns curdecisions: the data frame of decisions (don_id,rec_id,qty,remove_donor,remove_recipient)
# def dummy_strategy(donors,recipients):
#     # aggregate donors' and recipients' requests
#     donors = donors.groupby('don_id').agg({'date':'min','qty':sum})
#     donors = donors.reset_index()

#     recipients = recipients.groupby('rec_id').agg({'date':'min','qty':sum})
#     recipients = recipients.reset_index()

#     # match donor 0 with recipient 0
#     # throw away the other donors
#     decisions = pd.DataFrame(columns=['don_id','rec_id','qty','remove_donor'])
#     n = min(len(donors),len(recipients))
#     for i in range(n):
#         don = donors.iloc[i]
#         rec = recipients.iloc[i]
#         qty = min(don.qty,rec.qty)
#         # ship qty units of ppe from don.don_id to rec.rec_id
#         decisions.loc[len(decisions)] = [don.don_id,rec.rec_id,qty,True]
...
#         curdecisions = strategy(cur_don_ppe,cur_rec_ppe) # curDecisions: don_id,rec_id,qty,remove_donor

#         # update alldecisions
#         curdecisions['date'] = d2
#         curdecisions['ppe'] = ppe

#         # I still have to remove column remove_donor. But first we need to update the frames all_donors and all_recipients
#         requests_to_remove = []
#         for _,dec in curdecisions.iterrows():
...
#         curdecisions.drop(columns=['remove_donor'],inplace=True)
